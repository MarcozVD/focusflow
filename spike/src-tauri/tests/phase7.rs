...
fn core_loop_end_to_end() {
    ...
    assert_eq!(
        d.count().unwrap(),
        before,
        "generar propuesta no crea tareas"
    );
    ...
    let item = view
        .items
        .iter()
        .find(|i| i.title.contains("Examen"))
        .expect("ítem planificado");
    assert!(
        !item.sessions.is_empty(),
        "planifica sesiones de preparación"
    );
    ...
    let ev = tasks
        ...
        .expect("crea el evento del examen");
    assert_eq!(
        ev.start_at,
        view.understanding[0]
            .window_start
            .expect("ventana del examen")
    );
    let sessions: Vec<_> = tasks
        .iter()
        .filter(|t| t.metadata.contains(r#""plan_kind":"session""#))
        .collect();
...
fn rejected_plan_makes_no_changes() {
    ...
    let view = plan_from_text(
        &d,
        "Estudiar álgebra 1 hora mañana a las 18",
        &intents,
        &source,
    )
    .unwrap();
    ...
    assert_eq!(
        d.count().unwrap(),
        before,
        "rechazar no modifica el calendario"
    );
    let row = d.get_plan_proposal(view.id).unwrap().unwrap();
...
fn no_available_time_is_reported() {
    let d = db();
    // horizonte completo ocupado con tareas reales (CON hora: los marcadores
    // all-day son visuales y no bloquean tiempo, así que para agotar la
    // disponibilidad hay que usar compromisos duros 05:00–23:30 cada día;
    // la franja nocturna restante queda fuera del horario planificable)
    for day in 0..15i64 {
