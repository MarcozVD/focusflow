<script lang="ts">
  import { MONTHS_ES, DAYS_ES, completeTask, openTaskDetail, cat, type Task } from "./data.svelte";
  ...
  <div class="card {isCompleted ? 'done' : ''} {isOverdue ? 'overdue' : ''} {justCompleted ? 'pop' : ''}"
    style="--accent: {c.color}" role="button" tabindex="0"
    onclick={() => openTaskDetail(task)}
    onkeydown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); openTaskDetail(task); } }}
  >
  <button class="check" class:checked={isCompleted} onclick={(e) => { e.stopPropagation(); onComplete(); }} aria-label="Completar tarea">
    {#if isCompleted}
